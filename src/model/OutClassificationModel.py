...
from model.OutRobertaForSequenceClassification import OutRobertaForSequenceClassification
...
class OutClassificationModel:
    def __init__(
        ...
        # Tách cấu hình, mô hình, và tokenizer từ model_class_info
    ...
    def train_model(
        ...
        **kwargs,
        ...
        if "text" in train_df.columns and "labels" in train_df.columns:
            ...
            train_examples = [
                InputExample(i, text_a, text_b, label, feature)
                for i, (text_a, text_b, label, feature) in enumerate(
                    zip(train_df["text_a"], train_df["text_b"], train_df["labels"], train_df["feature"])
                )
            ]
        else:
            warnings.warn(
    ...
    def evaluate(self, eval_df, output_dir, multi_label=False, prefix="", verbose=True, silent=False, **kwargs):
        ...
        if "text" in eval_df.columns and "labels" in eval_df.columns:
            eval_examples = [
                InputExample(i, text, None, label, feature)
                for i, (text, label, feature) in enumerate(zip(eval_df["text"], eval_df["labels"], eval_df["feature"]))
            ]
        elif "text_a" in eval_df.columns and "text_b" in eval_df.columns:
            eval_examples = [
                InputExample(i, text_a, text_b, label, feature)
                for i, (text_a, text_b, label, feature) in enumerate(
                    zip(eval_df["text_a"], eval_df["text_b"], eval_df["labels"], eval_df["feature"])
                )
            ]
        else:
            warnings.warn(
    ...
    def load_and_cache_examples(
        ...
        dataset = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids, all_feature)
